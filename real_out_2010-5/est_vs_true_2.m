...
figure(4);
[inp_data_cont, k]=contour(raw_field, 'ShowText', 'on', 'LineWidth', 1.5);
...
ref_input = orig_input.sim_data.maps_norm(:,:,92);
...
EstCoords=coordDerive(outp_pos,.2,.001,5);
...
%EstCoords = eredmeny_control.eredmeny_control(:,:,3,443);
...
%[res_raw, sv_raw] = EstFieldForGADims(log(EstCoords), 45, 90, 0, 0);

%% NRMS error 
% 
% ii = 1;
% const_list = 1:100:500;
% NRMS_LIST = zeros(length(const_list), 1);
% 
% for constant = const_list 
%     disp('ERTEKEK')
%     disp(constant)
%     [res_raw, sv_raw] = EstFieldForGADims(log(EstCoords), 45, 90, 0, constant);
%     disp(max(max(res_raw)))
%     %MAE = compute_NRMS_new(res,Val);
%     NRMS = computeNRMS(ref_input,res_raw);
%     disp(NRMS)
%     NRMS_LIST(ii) = NRMS; %MAE;
%     ii = ii+1;
% end
% 
% NRMS = min(NRMS_LIST);
