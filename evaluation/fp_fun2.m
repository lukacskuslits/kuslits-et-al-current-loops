...
a=50+14.2*randn(n,dim);
b=50+14.2*randn(n,dim);
alfa=pi/3*randn(dim,1);

x=zeros(ceil(2*pi/step),dim);
for ijj=1:dim
x(:,ijj)=-pi():step:pi();
end
...
%     
%          end
%         diff=zeros(powell_dim');
%         diff=subsasgn(diff,S,fletcher-f);
%         target=zeros((dim-2)*nr,1);
%         
%       for elemf=1:nr  
%        for dimens=3:dim
%          
%           ind_pointer2=num2cell(IND(elemf,:));
%           for dimensb=3:dim
%           S.subs{dimensb}=ind_pointer2{dimensb};
%           end
%           
%           diffsect=subsref(diff,S);
%           
%           target(dimens*elemf)=1/abs(norm(diffsect));
%           
%        end
%       end
%        
%        TARGET=sum(target);
    ...
    MINF=(min(min(min((fletcher)))));
    for elemf=1:nr
     ind_pointer=num2cell(IND(elemf,:));     
     indx=sub2ind(size(fletcher),ind_pointer{:}); 
     if fletcher(indx)==MINF
         ind_pointer, elemf1=elemf, x(IND(elemf1,1),1), x(IND(elemf1,2),2), x(IND(elemf1,3),3), x(IND(elemf1,4),4)
         alfa
     end
    end
