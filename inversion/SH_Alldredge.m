...
for ii = 1:nloops
    if ~any(strcmp(fix, 'alpha'))
         par_nr = 1;
         alpha = structure_input('alpha', loop_pars, ii, loop_nr, par_nr, nloops, fix);
    else
        alpha=alpha0(ii);
    end
    %
    ...
    if ~any(strcmp(fix, 'theta'))
        par_nr = 4;
        theta = structure_input('theta', loop_pars, ii, loop_nr, par_nr, nloops, fix);
    else 
        theta = theta0(ii);
    end
    %
    if ~any(strcmp(fix, 'phi'))
        par_nr = 5;
        phi = structure_input('phi', loop_pars, ii, loop_nr, par_nr, nloops, fix);
    else
        phi = phi0(ii);
    end
    %
